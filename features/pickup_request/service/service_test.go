...
func (m *MockPickupRequestData) GetAllPickupRequest(role string, id uint) ([]pickuprequest.PickupRequestInfo, error) {
...
func TestGetAllPickupRequestCollector(t *testing.T) {
    mockData := new(MockPickupRequestData)
    service := New(mockData)
    ...
    role := "Collector"
    id := uint(0) // Untuk collector, id tidak digunakan

    mockData.On("GetAllPickupRequest", role, id).Return(expectedResult, nil)

    result, err := service.GetAllPickupRequest(role, id)
    assert.Nil(t, err)
    assert.Equal(t, expectedResult, result)
    mockData.AssertExpectations(t)
}

func TestGetAllPickupRequestUser(t *testing.T) {
    mockData := new(MockPickupRequestData)
    service := New(mockData)
    expectedResult := []pickuprequest.PickupRequestInfo{
        {
